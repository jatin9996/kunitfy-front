...
import ipfs from "../../ipfsClient"; // Import IPFS client
...
const CreateCollection = (props) => {
  ...
  const [uploadMetadata, setUploadMetadata] = useState(false);
  ...
  const handleCollectionCreation = async () => {
    ...
    if (res1.status === 1) {
      ...
      let metadataResult;
      if (uploadMetadata && metadataFile) {
        metadataResult = await ipfs.add(metadataFile);
      } else {
        const metadata = {
          title,
          description,
      ...
      fd.append("ipfsImageHash", imageResult.path);
      fd.append("ipfsMetadataHash", metadataResult.path);
    }
    setLoading(true);
              ...
              <div className="field-set">
                ...
                <div className="form-check form-switch">
                  <input
                    ...
                    type="checkbox"
                    id="uploadMetadataSwitch"
                    checked={uploadMetadata}
                    onChange={() => setUploadMetadata(!uploadMetadata)}
                  />
                  <label className="form-check-label" htmlFor="uploadMetadataSwitch">
                ...
                {uploadMetadata && (
                  <div className="d-create-file">
                    <h5>Upload Metadata JSON</h5>
                    <div className="browse">
                      <input
                        type="button"
                        id="get_metadata_file"
                        className="btn-main"
                        value="Browse"
                      />
                      <input
                        id="upload_metadata_file"
                        type="file"
                        onChange={(e) => onMetadataChange(e)}
                      />
                    </div>
                  </div>
                )}
